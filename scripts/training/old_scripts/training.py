...
def train():
    ...
    X_valid = torch.tensor(valid_trigram_vecs, dtype=torch.float32)
    Y_valid = torch.tensor(valid_labels, dtype=torch.int64)
    ...
    valid_counts = max(counts)
    valid_imbalance = max(counts) / Y_valid.shape[0]
